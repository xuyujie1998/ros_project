...
class ObstacleAvoidance:
    def __init__(self):
        ...
        self.PUBLISH_RATE_HZ = 2    # PUBLISH RATE 2 times/sec
        
        self.cloest_obtacle_distance
        
        self.sub = rospy.Subscriber('scan', LaserScan, self.laserScanCallback)  # define subscriber with name "scan"
        self.sub2 = rospy.Subscriber('seeRedLine', CameraScan, self.redlineCallback)  # define subscriber with name "scan"
        self.sub3 = rospy.Subscriber('lookup', CameraScan3, self.laserScanCallback3)  # define subscriber with name "scan"
        
        self.pub = rospy.Publisher('cmd_vel', Twist, queue_size=10)   # define publisher with name "cmd_vel" # queue: cache of message
        self.pub2 = rospy.Publisher('red_line_cmd_vel', Twist, queue_size=10)   # define publisher with name "cmd_vel" # queue: cache of message
        self.pub3 = rospy.Publisher('lights_cmd_vel', Twist, queue_size=10)   # define publisher with name "cmd_vel" # queue: cache of message
    ...
    def printDefaultValues(self):   # print default parameter values
        ...
        print('PUBLISH_RATE_HZ = ' + str(self.PUBLISH_RATE_HZ) + '\t# PUBLISH_RATE_HZ')
    ...
    def getLinearVelocity(self, d, LV, angle):
        print('getLinearVelocity(d='+str(d)+',LV='+str(LV)+',angle='+str(angle)+'):')
    ...
    def getAngularVelocity(self, d, angle):
        print('getAngular(d='+str(d)+', angle='+str(angle)+'):')
    ...
    def laserScanCallback(self, msg):
        ...
        if cloest_obtacle_distance < self.D_MAX:     # If found any obstacle
            ...
            if angle >= 0 and angle < 90:
                LV = self.getLinearVelocity(d, LV, angle, red, green, blue)
                AV = 0 - self.getAngularVelocity(d, angle, red, green, blue)
            elif angle >= 90 and angle < 180:
                LV = self.getLinearVelocity(d, LV, 180-angle, red, green, blue)
                AV = self.getAngularVelocity(d, 180-angle, red, green, blue)
            elif angle >= -90 and angle < 0:
                LV = self.getLinearVelocity(d, LV, 0-angle, red, green, blue)
                AV = self.getAngularVelocity(d, 0-angle, red, green, blue)
            elif angle >= -180 and angle < -90:
                LV = self.getLinearVelocity(d, LV, 180+angle, red, green, blue)
                AV = 0 - self.getAngularVelocity(d, 180+angle, red, green, blue)
    ...
    def redlineCallback(self): # redline detection callback
        .....
         if self.cloest_obtacle_distance < self.D_MAX:
            // stop subscribing red_line
         else 
            // restart to subscribe red_line
    
    def laserScanCallback3():
        .....
